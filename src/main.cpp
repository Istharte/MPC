...
// Evaluate a slope of tangent of polynomial.
...
// transform ground coordinate to vehicle coordinate.
vector<double> TransformObs(double o_x, double o_y, double p_x, double p_y, double p_theta) {
    
    double dx = o_x - p_x;
    double dy = o_y - p_y;
    
    double r_x = dx * cos(-p_theta) - dy * sin(-p_theta);
    double r_y = dx * sin(-p_theta) + dy * cos(-p_theta);
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          double throttle_value = vars[1] / 100.0;
          ...
          for (int i = 0; i < 10; i++) {
            ...
            mpc_y_vals.push_back(vars[12 + i]);
          }
